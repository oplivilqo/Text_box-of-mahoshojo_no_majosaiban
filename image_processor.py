...
class ImageProcessor:
    ...
    def _load_background(self, background_index: int) -> Image.Image:
        ...
        if background_index not in self.background_cache:
            ...
            self.background_cache[background_index] = Image.open(background_path).convert("RGBA")
        return self.background_cache[background_index].copy()
    
    def _load_character_image(self, character_name: str, emotion_index: int) -> Image.Image:
        """加载角色图片"""
    ...
    def generate_base_image_with_text(self, character_name: str, background_index: int, 
                                    emotion_index: int) -> Image.Image:
        """生成带角色文字的基础图片"""
    ...
    def generate_image_directly(self, character_name: str, background_index: int, 
        ...
        """直接生成图片"""
        # 获取预生成的基础图片
        base_image = self.generate_base_image_with_text(character_name, background_index, emotion_index)
    ...
    def generate_image_fast(self, character_name: str, background_index: int, 
                           emotion_index: int, text: str = None, 
                           content_image: Image.Image = None,
                           font_path: str = None) -> bytes:
        """快速生成图片 - 使用当前预览的基础图片"""
    ...
    def generate_preview_image(self, character_name: str, background_index: int, 
        ...
        try:
            ...
            # 缓存当前基础图片用于快速生成
            self.current_base_image = base_image.copy()
            self.current_base_key = f"{character_name}_{background_index}_{emotion_index}"
